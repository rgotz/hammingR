...
nome.arquivo <- readline("Entre o nome do arquivo deste diretorio (com extensao) a ser aplicado Hamming(16,8): ")
leitura.arquivo <- file(nome.arquivo, "rb")

arquivo.tamanho <- file.info(nome.arquivo)$size

arquivo.bin.corrigido = raw()
...
for (byte.arq.bin in 1:arquivo.tamanho){
  ...
  (teste <- packBits(gerador.hamming(arquivo.byte), "raw"))
  writeBin(teste, escrita.arquivo)
